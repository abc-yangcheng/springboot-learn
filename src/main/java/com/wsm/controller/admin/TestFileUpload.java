package com.wsm.controller.admin;
...
//        System.out.println("文件描述：" + description);  @RequestParam("description") String description,
        //如果选择了上传文件，将文件上传到指定的目录uploadFiles
        if (!myfile.isEmpty()) {
            //上传文件路径request.getServletContext().getRealPath("/uploadFiles/")
    ...
    public String showDownLoad(HttpServletRequest request, Model model) throws FileNotFoundException {
        String path = ResourceUtils.getURL("classpath:").getPath();
